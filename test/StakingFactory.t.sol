...
function testEmergencyWithdraw() public {
    ...
    uint256 stakingPoolBalanceBefore = lpToken.balanceOf(address(stakingPool));
    console2.log("Staking pool balance before emergencyWithdraw:", stakingPoolBalanceBefore);

    stakingFactory.emergencyWithdraw(0);

    uint256 stakingPoolBalanceAfter = lpToken.balanceOf(address(stakingPool));
    console2.log("Staking pool balance after emergencyWithdraw:", stakingPoolBalanceAfter);
...
function testAddMultiplePools() public {
    ...
    stakingPool2 = new StakingPool(address(stakingFactory), address(rewardToken), owner, address(lpToken2));
    stakingPool3 = new StakingPool(address(stakingFactory), address(rewardToken), owner, address(lpToken3));
...
function testWithdrawWithCorrectSharesCalculation() public {
    ...
    assertEq(initialShares, amount - (amount * stakingPool.entranceFeeFactor()) / 10000);
...
function testUserDepositAndWithdraw() public {
    ...
    uint256 entranceFee = amount * stakingPool.entranceFeeFactor() / 10000;
...
function testDifferentEntryFeesForDifferentPools() public {
    ...
    uint256 entranceFee1 = amount1 * stakingPool.entranceFeeFactor() / 10000;
    uint256 entranceFee2 = amount2 * stakingPool2.entranceFeeFactor() / 10000;
...
function testMultipleUsersDepositAndClaimRewards() public {
    vm.startPrank(owner);
    stakingFactory.add(100, lpToken, true, address(stakingPool));
    vm.stopPrank();
    ...
    uint256 amount1 = 1000 * DECIMAL;
    uint256 amount2 = 500 * DECIMAL;
    uint256 rewardAmount = 100 * DECIMAL * 1e12;
    ...
    vm.startPrank(user);
    stakingFactory.claimReward(0);
    uint256 user1Rewards = rewardToken.balanceOf(user);
    vm.stopPrank();

    // User 2 claims rewards
    vm.startPrank(user2);
    stakingFactory.claimReward(0);
    uint256 user2Rewards = rewardToken.balanceOf(user2);
    vm.stopPrank();
    uint256 totalRewards = rewardAmount * 10;
    uint256 user1ExpectedReward = totalRewards * amount1 / (amount1 + amount2);
    uint256 user2ExpectedReward = totalRewards * amount2 / (amount1 + amount2);

    assertEq(user1Rewards, user1ExpectedReward);
    assertEq(user2Rewards, user2ExpectedReward);
}
